...
GRAY = (128, 128, 128)
BLUE = (0, 0, 255)
GREEN = (0, 255, 0)  # Za hint označavanje

class SudokuGameGUI:
    def __init__(self):
        # Kreiranje prozora igre
        self.window = pygame.display.set_mode((WINDOW_SIZE, WINDOW_SIZE))
        pygame.display.set_caption("Sudoku sa Backtracking Solverom")
        ...
        self.hint_cell = None  # Polje za koje je dat hint
    ...
    def get_hint(self):
        # Generisanje predloga za sledeći potez
        ...
        while running:
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    # Selekcija polja klikom
                    pos = pygame.mouse.get_pos()
                    x, y = pos[1] // CELL_SIZE, pos[0] // CELL_SIZE
                    self.selected = (x, y)
                    self.hint_cell = None  # Resetovanje hint označavanja
                
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_h:
                        # Aktiviranje hint sistema
                        hint = self.get_hint()
                        if hint:
                            self.hint_cell = hint
